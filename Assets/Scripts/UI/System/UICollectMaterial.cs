...
public delegate void GemstoneEventHandler(int value,CollectMaterialGemstoneNode node, Collider2D collision = null);
public delegate void EnemyEventHandler(Collider2D collision = null,CollectMaterialEnemyNode node = null);
public class UICollectMaterial : UIBase, IPointerDownHandler, IPointerUpHandler
{
    ...
    {
        playerNode.transform.position = grid2D[0, 0].transform.position;
        for (int i = 0; i < currEnemyList.Count; i++)
        {
            PushEnemyItem(currEnemyList[i]);
        }
